...
import DeviceImgSmartToggle from "./DeviceImgSmartToggle";
...
function DeviceIconSelector(deviceType, connectedDeviceList, targetDeviceMacAddres) {
  ...
  for(let DeviceInfo of connectedDeviceList){
      if(DeviceInfo.macAddress === targetDeviceMacAddres){  // 해당 Target 기기가 연결된 상태일 때때
        console.log(`${i} TARGET ${DeviceInfo.macAddress} === ${targetDeviceMacAddres} `)
        console.log(`${deviceType} <-- 연결상태 Green`);
  ...
  return (
    ...
    <>
      ...
      <div
        ...
        onClick={() => console.log("111111111111111111")} // 함수로 감싸기
        // onClick={setDeviceManagingFormOpen(true)} // Card 클릭 시 모달 띄우기
        className="flex flex-col"
      >

        {/* Device Name */}
        <p className="mt-0 mb-1 ml-2 text-left text-gray-100">{deviceType}</p>
        <span className="text-white">{JSON.stringify(connectedDeviceList,null,2)}</span>
